...
/* Subroutine */ int sgemv_(char *trans, integer *m, integer *n, real
*alpha, 
 ...
 *
 *        y := alpha*A*x + beta*y,   or   y := alpha*A'*x +
 *        beta*y,   
 *
 *   where alpha and beta are scalars, x and y are
 *   vectors and A is an   
 *       m by n matrix.   
 *
 *      Parameters   
 ...
 *      Unchanged on exit.   
 *
 *     X - REAL array of DIMENSION at least ( 1 + ( n - 1 )*abs( INCX ) )
 *      when TRANS = 'N' or 'n'  
...
#define A(I,J) a[(I)-1 + ((J)-1)* ( *lda)]
    ...
    if (info != 0) {
        printf("error in calling sequence of SGEMV\n");
	return -1;
    }
